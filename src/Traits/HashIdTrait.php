...
namespace Maatwebsite\Excel\Traits;

use Illuminate\Support\Facades\Config;
use Maatwebsite\Excel\Exceptions\IncorrectIdException;
...
/**
 * Trait HashIdTrait
 * @package Maatwebsite\Excel\Traits
 */
trait HashIdTrait
{
	 ...
	 * @var  array
	 */
	private $skippedEndpoints = [//        'orders/{id}/external',
	];
	 ...
	 * @return  array
	 */
	protected function decodeHashedIdsBeforeValidation(array $rows, array $decode)
	{

		// the hash ID feature must be enabled to use this decoder feature.
		if (Config::get('apiato.hash-id') && isset($decode) && !empty($decode))
		{
			// iterate over each key (ID that needs to be decoded) and call keys locator to decode them
			foreach ($decode as $key)
			{
				foreach ($rows as $index => $row){
					$rows[$index] = $this->locateAndDecodeIds($row, $key);
				}
			}
		}
	 ...
	 * @return  mixed
	 */
	private function locateAndDecodeIds($data, $key)
	{
		// split the key based on the "."
		$fields = explode('.', $key);
		// loop through all elements of the key.
		return $this->processField($data, $fields);
	}
